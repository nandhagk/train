...
function _tryOverload(funcName, args, namedArgs, typeSpec, funcDef) {
	if (
		...
		(namedArgs === undefined && funcDef.namedArgs && Object.values(funcDef.namedArgs).some((arg) => !arg.optional))
	) {
		return null;
	}
	...
	for (let i = 0; i < funcDef.args.length; i++) {
		...
		if (arg === undefined) {
			...
			if (argDef.setoftype) {
				paramCardinalities.push(funcDef.preservesOptionality ? cardutil.overrideUpperBound(arg.__cardinality__, 'One') : Cardinality.One);
			} else {
	...
	if (funcName === 'if_else') {
		cardinality = cardutil.multiplyCardinalities(
			cardutil.orCardinalities(positionalArgs[0].__cardinality__, positionalArgs[2].__cardinality__),
		...
		cardinality = funcDef.returnTypemod === 'SetOfType' ? Cardinality.Many : cardutil.multiplyCardinalitiesVariadic(paramCardinalities);
	...
	const descendants = [
		...new Set(
			[...typeSpec.values()]
				...
				.flatMap((type) => (type.is_abstract ? getDescendantNames(typeSpec, type.id) : [nameRemapping[type.name], type.name]))
		)
	];
