	...
	Set extends TypeSet<ScalarType<'std::json'>>
		? {
				...
				destructure<T extends TypeSet<ScalarType<'std::str'>> | string>(
					...
					Set['__element__'],
					cardutil.multiplyCardinalities<Set['__cardinality__'], T extends TypeSet ? T['__cardinality__'] : Cardinality.One>
				>;
			}
		: unknown;
			...
			: Element extends TypeSet
				...
				: Pointer extends LinkDesc
					? Element extends object
						? computeTsTypeCard<
								computeObjectShape<Pointer['target']['__pointers__'] & Pointer['properties'], Element>,
								Pointer['cardinality']
							>
						: never
					: never;
...
type $arrayLikeIndexify<Set extends TypeSet> = Set['__element__'] extends ArrayType | ScalarType<'std::str'> | ScalarType<'std::bytes'>
	? {
			[index: number]: $expr_Operator<
				getPrimitiveBaseType<Set['__element__'] extends ArrayType<infer El> ? El : Set['__element__']>,
				Set['__cardinality__']
			>;
			...
			slice<S extends TypeSet<ScalarType<'std::number'>> | number, E extends TypeSet<ScalarType<'std::number'>> | number | undefined | null>(
				...
				cardutil.multiplyCardinalities<
					cardutil.multiplyCardinalities<Set['__cardinality__'], S extends TypeSet ? S['__cardinality__'] : Cardinality.One>,
			...
			slice<E extends TypeSet<ScalarType<'std::number'>> | number | undefined | null>(
				...
				getPrimitiveBaseType<Set['__element__']>,
				cardutil.multiplyCardinalities<Set['__cardinality__'], E extends TypeSet<any, infer C> ? C : Cardinality.One>
			>;
		}
	: unknown;
...
export type tupleElementsToTupleType<T extends typeutil.tupleOf<TypeSet>> =
	baseTupleElementsToTupleType<T> extends BaseTypeTuple ? TupleType<baseTupleElementsToTupleType<T>> : never;
...
export type tupleElementsToCardTuple<T> =
	baseTupleElementsToCardTuple<T> extends [Cardinality, ...Cardinality[]] ? baseTupleElementsToCardTuple<T> : never;
...
type inferNamedTupleCardinality<Shape extends NamedTupleLiteralShape> = [Cardinality.Many] extends [shapeCardinalities<Shape>]
	? Cardinality.Many
...
export interface MultiRangeType<Element extends ScalarType = ScalarType, Name extends string = `multirange<${Element['__name__']}>`>
	extends BaseType {
	__name__: Name;
...
export type orLiteralValue<Set extends TypeSet> =
	| Set
	| (Set['__element__'] extends ObjectType ? never : computeTsType<Set['__element__'], Set['__cardinality__']>);
type ScalarTypeToTsType<T extends ScalarType, isParam extends boolean> = isParam extends true ? T['__tsargtype__'] : T['__tsconsttype__'];
type ArrayTypeToTsType<Type extends BaseArrayType, isParam extends boolean> = isParam extends true
	? readonly BaseTypeToTsType<Type['__element__'], isParam>[]
	: BaseTypeToTsType<Type['__element__'], isParam>[];
export type BaseTypeToTsType<Type extends BaseType, isParam extends boolean = false> = BaseType extends Type
	? unknown
