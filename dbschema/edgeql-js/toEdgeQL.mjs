...
export function $toEdgeQL() {
	...
	for (const [expr, refData] of seen) {
		...
		if (
			(expr.__kind__ === ExpressionKind.Select || expr.__kind__ === ExpressionKind.Update || expr.__kind__ === ExpressionKind.Group) &&
			expr.__scope__ &&
			...
			withVars.set(scopeVar, {
				...
				scope: withBlock,
				childExprs: new Set(),
				scopedExpr: expr.__element__.__kind__ === TypeKind.object ? expr.__expr__ : undefined
			});
		}
		if (refData.refCount > 1 || refData.boundScope || refData.aliases.length > 0) {
			...
			if (!withBlock) {
				...
				if (parentScopes.some((scope) => scope == null)) {
					throw new Error(`Cannot extract repeated expression into 'WITH' block, ` + `expression used outside of 'WITH'able expression`);
				}
	...
	if (
		edgeQL.startsWith('(') &&
		edgeQL.endsWith(')') &&
		!(this.__kind__ === ExpressionKind.Tuple || this.__kind__ === ExpressionKind.NamedTuple || this.__kind__ === ExpressionKind.Literal)
	) {
	...
	switch (expr.__kind__) {
		...
		case ExpressionKind.PathNode:
			if (expr.__parent__) {
				...
				if (expr.__kind__ === ExpressionKind.PathLeaf && expr.__parent__.linkName.startsWith('@')) {
		...
		case ExpressionKind.Update: {
			...
			if (expr.__kind__ === ExpressionKind.Select) {
				if (isObjectType(expr.__element__) && expr.__element__.__shape__ !== expr.__expr__.__element__.__shape__) {
...
function renderEdgeQL(_expr, ctx, renderShape = true, noImplicitDetached = false) {
	...
	if (expr.__kind__ === ExpressionKind.With) {
		...
		return `(WITH\n${expr.__params__
			.map((param) => {
				...
				if (param.__isComplex__) {
					...
					cast = cast.includes('std::decimal') ? `<${cast}><${cast.replace(/std::decimal/g, 'std::str')}>` : `<${cast}>`;
		...
		if (exprs.every((ex) => ex.__element__.__kind__ === TypeKind.object) || exprs.every((ex) => ex.__element__.__kind__ !== TypeKind.object)) {
			...
			throw new Error(`Invalid arguments to set constructor: ${exprs.map((ex) => ex.__element__.__name__).join(', ')}`);
		}
		...
		if (isObjectType(expr.__element__)) {
			...
			lines.push(`SELECT${selectionTarget === 'DETACHED std::FreeObject' ? '' : ` ${selectionTarget}`}`);
			...
			const needsScalarVar =
				(expr.__modifiers__.filter || expr.__modifiers__.order_by || expr.__modifiers__.offset || expr.__modifiers__.limit) &&
		...
		return '(' + withBlock + lines.join(' ') + (modifiers.length ? '\n' + modifiers.join('\n') : '') + ')';
		...
		const combinedBlock = [...groupingSet.__exprs__.map(([k, v]) => `  ${k} := ${renderEdgeQL(v, ctx)}`)];
		...
		selectStatement.push(`WITH\n${[...unscopedWithBlock, ...scopeExprVar].join(',\n')},
  ${selectTarget} := (
...
function shapeToEdgeQL(shape, ctx, type = null, keysOnly = false, injectImplicitId = true) {
	...
	for (const key in shape) {
		...
		if (keysOnly) {
			addLine(q(key, false) + (isObjectType(val.__element__) ? `: ${shapeToEdgeQL(val.__element__.__shape__, ctx, null, true)}` : ''));
			continue;
		}
...
function literalToEdgeQL(type, val) {
	...
	if (typename === 'std::json') {
		...
		return `std::range(${val.lower === null ? `<${elTypeName}>{}` : literalToEdgeQL(elType, val.lower)}, ${
