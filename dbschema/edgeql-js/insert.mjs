...
export function $normaliseInsertShape(root, shape, isUpdate = false) {
	...
	for (const [key, _val] of _shape) {
		...
		const isMulti = pointer.cardinality === Cardinality.AtLeastOne || pointer.cardinality === Cardinality.Many;
	...
	if (typeof shape !== 'object') {
		throw new Error(
			`invalid insert shape.${
				typeof shape === 'function' ? ' Hint: Insert shape is expected to be an object, ' + 'not a function returning a shape object.' : ''
			}`
		);
	}
	const expr = {
