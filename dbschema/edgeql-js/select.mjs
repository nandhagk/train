...
export function $handleModifiers(modifiers, params) {
	...
	if (modifiers.filter_single) {
		...
		if (fs.__element__) {
			...
			const exprs = Object.keys(fs).map((key) => {
				const val = fs[key].__element__ ? fs[key] : literal(root.__element__['__pointers__'][key]['target'], fs[key]);
	...
	if (modifiers.order_by) {
		...
		mods.order_by = orderExprs.map((expr) => (typeof expr.__element__ === 'undefined' ? expr : { expression: expr }));
	}
	if (modifiers.offset) {
